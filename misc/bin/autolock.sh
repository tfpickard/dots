...
i3l="i3lock-fancy"
i3l_cmd="$HOME/bin/i3lock-fancy --ignore-empty-password \
    --inactivity-timeout=10 --nofork -F"
echo $i3l_cmd
...
locked() { pkill -0 --euid "$(id -u)" --exact $i3l; }
...
case "$cmd" in
  ...
  notify)
    ...
    notify-send --urgency="normal" --app-name="xautolock" \
      --icon='/usr/share/icons/la-capitaine/actions/22x22-dark/system-lock-screen.svg' \
      -- "Screen Lock" "$secs"
    ;;
